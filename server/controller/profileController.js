...
const get_orders = async (req, res) => {
  try {
    ...
    const orderCount = await Order.find({ user_id: id }).countDocuments();
...
const add_address = async (req, res) => {
  try {
    ...
    const { name, state, country, pincode, landmark, title } = req.body;
...
const edit_address = async (req, res) => {
  try {
    ...
    const update_address = await Address.updateOne(
      { _id: id },
      { $set: updated_address }
    );
...
const change_password = async (req, res) => {
  ...
  const check_current_password = await bcrypt.compare(
    currentPass,
    user_password
  );
  if (check_current_password) {
    ...
    if (newPass === confirmNewPass) {
      ...
      req.flash(
        "invalidNewPassword",
        "new password and confim password should be same !"
      );
      return res.status(403).redirect(`/security/${id}`);
    }
  } else {
